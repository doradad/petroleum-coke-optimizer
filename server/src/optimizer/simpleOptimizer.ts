...
export class SimpleOptimizer {
  optimize(products: Product[], constraints: Constraints): OptimizationResult {
    ...
    // 枚举所有可能的单一产品解
    const singleProductSolutions: OptimizationResult[] = [];
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (product.sulfur <= constraints.sulfur &&
        ...
        const solution: OptimizationResult = {
          products: [{
            product,
            ratio: 1.0
          }],
  ...
  private tryTwoProductMix(p1: Product, p2: Product, constraints: Constraints): OptimizationResult | null {
    // 对于两产品混合，我们需要找到最优比例
    ...
    if (bestRatio >= 0) {
      ...
      if (r1 > 0.001) productsInSolution.push({ product: p1, ratio: r1 });
      if (r2 > 0.001) productsInSolution.push({ product: p2, ratio: r2 });
