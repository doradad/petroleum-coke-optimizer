 ...
 */
export class DirectSearchOptimizer {
  ...
  private trySingleProducts(products: Product[], constraints: Constraints): OptimizationResult[] {
  ...
  private tryTwoProductCombinations(products: Product[], constraints: Constraints, maxProducts: number): OptimizationResult[] {
    const results: OptimizationResult[] = [];
  ...
  private tryThreeProductCombinations(products: Product[], constraints: Constraints, maxProducts: number): OptimizationResult[] {
    const results: OptimizationResult[] = [];
  ...
  private optimizeTwoProducts(p1: Product, p2: Product, constraints: Constraints): OptimizationResult | null {
    let bestRatio = -1;
    let bestCost = Infinity;
  ...
  private optimizeThreeProducts(p1: Product, p2: Product, p3: Product, constraints: Constraints): OptimizationResult | null {
  ...
  private tryFourProductCombinations(products: Product[], constraints: Constraints, currentBest: number): OptimizationResult[] {
    const results: OptimizationResult[] = [];
      ...
      for (let j = i + 1; j < topProducts.length - 2; j++) {
        for (let k = j + 1; k < topProducts.length - 1; k++) {
          for (let l = k + 1; l < topProducts.length; l++) {
            // 快速预检：理论最低成本是否有希望
            const minPossibleCost = Math.min(
              topProducts[i].price, topProducts[j].price, 
              topProducts[k].price, topProducts[l].price
            );
            if (minPossibleCost >= currentBest) continue;
            
            const result = this.optimizeFourProducts(
              topProducts[i], topProducts[j], topProducts[k], topProducts[l], constraints
            );
  ...
  private optimizeFourProducts(p1: Product, p2: Product, p3: Product, p4: Product, constraints: Constraints): OptimizationResult | null {
  ...
  private randomSearch(products: Product[], constraints: Constraints, iterations: number): OptimizationResult[] {
    const results: OptimizationResult[] = [];
    const n = products.length;
  ...
  private generateRandomRatios(n: number): number[] {
    // 生成随机比例，确保总和为1
  ...
  private satisfiesConstraints(ratios: number[], products: Product[], constraints: Constraints): boolean {
  ...
  private createResult(products: Product[], ratios: number[], constraints: Constraints): OptimizationResult {
  ...
  // 专门针对三产品组合的随机搜索
  private specialThreeProductRandomSearch(products: Product[], constraints: Constraints, iterations: number): OptimizationResult[] {
    const results: OptimizationResult[] = [];
    const n = products.length;
  ...
  private selectRandomThreeProducts(n: number): number[] {
  ...
  private generateDirichletRatios(k: number): number[] {
  ...
  private generateGamma(alpha: number, beta: number): number {
    if (alpha < 1) {
