...
import { LinearProgrammingSolver, LPResult } from './lpSolver';
...
export class LinearProgrammingOptimizer {
  private solver = new LinearProgrammingSolver();
   ...
   * 6. x_i ≥ 0
   */
  private buildLPProblem(products: Product[], constraints: Constraints) {
    const n = products.length;
  ...
  private createOptimalResult(products: Product[], solution: number[], constraints: Constraints): OptimizationResult {
    const n = products.length;
