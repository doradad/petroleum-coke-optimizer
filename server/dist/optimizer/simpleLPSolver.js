...
class SimpleLPSolver {
     ...
     */
    enumerateBasicFeasibleSolutions(products, constraints) {
        ...
        // 进度报告函数
        const reportProgress = (stage, currentBest) => {
        ...
        for (let ii = 0; ii < n && threeProductCount < maxThreeProductCombinations; ii++) {
            ...
            for (let jj = ii + 1; jj < n && threeProductCount < maxThreeProductCombinations; jj++) {
                ...
                for (let kk = jj + 1; kk < n && threeProductCount < maxThreeProductCombinations; kk++) {
                    ...
                    const threeProductSolution = this.optimizeThreeProducts(products[i], products[j], products[k], i, j, k, n, constraints);
    ...
    optimizeTwoProducts(p1, p2, i1, i2, n, constraints) {
        let bestRatio = null;
        let bestCost = Infinity;
        // 网格搜索最优比例
    ...
    optimizeThreeProducts(p1, p2, p3, i1, i2, i3, n, constraints) {
        let bestRatios = null;
        let bestCost = Infinity;
        // 平衡优化：适中的精度确保找到正确解
    ...
    optimizeFourProducts(p1, p2, p3, p4, i1, i2, i3, i4, n, constraints) {
        let bestRatios = null;
        let bestCost = Infinity;
        // 粗网格搜索四产品组合
    ...
    // 连续优化：在当前最优解附近进行梯度搜索
    continuousOptimization(products, currentSolution, constraints, currentCost) {
        const n = products.length;
    ...
    calculateCost(solution, products) {
        let cost = 0;
        for (let i = 0; i < solution.length && i < products.length; i++) {
    ...
    createResult(products, solution, constraints, totalCost) {
        const tolerance = 1e-6;
        // 过滤掉比例极小的产品
...
exports.SimpleLPSolver = SimpleLPSolver;
