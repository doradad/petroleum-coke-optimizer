...
router.post('/optimize', async (req, res) => {
    try {
        ...
        // 获取所有产品
        const products = await database_1.default.getAllProducts();
        if (products.length === 0) {
            return res.status(400).json({ error: '没有产品数据，请先上传产品清单' });
        }
        // 验证约束可行性
        const validation = optimizer_1.default.validateConstraints(products, constraints);
        ...
        // 执行优化
        const result = optimizer_1.default.optimize(products, constraints);
        if (!result.feasible) {
            return res.status(400).json({
